...
KOSIS 공개 통계표와 e-지방지표 응답을 이용해 아래 표준 CSV를 생성했다.

- `data/optional_kosis/additional_factors_panel_2015_2025.csv`
- `data/optional_kosis/population_change_panel_2015_2025.csv`
- `data/optional_kosis/optional_kosis_sources.csv`

실제 수집 변수는 `조혼인율`, `혼인건수`, `평균초혼연령`, `사교육비`, `경력단절여성비율`, `주택매매가격지수`, `주택전세가격지수`, `청년순이동률`, `자연증가율`이다. `경력단절여성인구`는 규모 효과를 줄이기 위해 15~54세 기혼여성 대비 비율로 변환했다. 주택 가격지수는 월별 자료를 연평균으로 집계했다.

선택 이유는 기존 9개 요인과 개인소득으로 설명하기 어려운 경로를 보완하는 것이다. 혼인 지표는 출산율의 인구학적 중간 경로, 사교육비는 예상 양육비 부담, 주택 가격지수는 주거안정률로 잡히지 않는 가격 압박, 경력단절여성비율은 출산 이후 경력 비용, 청년순이동률은 지역별 청년 유출입과 수도권 집중을 확인하기 위한 지표다.

해석 원칙은 기존 메인 분석과 분리하는 것이다. 추가 변수는 교수님 강의 방식의 기본 complete-case/PCA/PCR 결론을 대체하지 않고, 확장 민감도 분석으로만 제시한다. 특히 혼인 지표는 합계출산율과 가까운 중간 메커니즘이므로 정책 원인처럼 단정하지 않는다.
