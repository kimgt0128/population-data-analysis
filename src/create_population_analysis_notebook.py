...
cells = [
    ...
    code(
        ...
        standard_optional_vars = set()

        OPTIONAL_STANDARD_PANEL_PATH = DATA_DIR / "optional_kosis" / "additional_factors_panel_2015_2025.csv"
        OPTIONAL_STANDARD_SOURCE_PATH = DATA_DIR / "optional_kosis" / "optional_kosis_sources.csv"

        if OPTIONAL_STANDARD_PANEL_PATH.exists():
            standard_optional_panel = pd.read_csv(OPTIONAL_STANDARD_PANEL_PATH)
            standard_optional_panel.columns = [nfc(str(col).strip()) for col in standard_optional_panel.columns]
            standard_optional_panel["지역"] = standard_optional_panel["지역"].map(normalize_region)
            standard_optional_panel["연도"] = pd.to_numeric(standard_optional_panel["연도"], errors="coerce").astype("Int64")
            standard_optional_panel = standard_optional_panel.dropna(subset=["지역", "연도"]).copy()
            standard_optional_panel["연도"] = standard_optional_panel["연도"].astype(int)
            standard_optional_panel = keep_sido_rows(standard_optional_panel)

            for variable in [col for col in standard_optional_panel.columns if col not in ["지역", "연도"]]:
                standard_optional_panel[variable] = pd.to_numeric(standard_optional_panel[variable], errors="coerce")
                extracted = standard_optional_panel[["지역", "연도", variable]].dropna(subset=[variable]).copy()
                if extracted.empty:
                    continue
                optional_frames.append(extracted)
                standard_optional_vars.add(variable)
                optional_status_rows.append(
                    {
                        "변수": variable,
                        "상태": "표준CSV추출",
                        "파일": OPTIONAL_STANDARD_PANEL_PATH.relative_to(PROJECT_DIR).as_posix(),
                        "관측치수": int(extracted.shape[0]),
                        "최소연도": int(extracted["연도"].min()),
                        "최신연도": int(extracted["연도"].max()),
                        "메모": "download_optional_kosis_data.py로 생성한 표준 패널 사용",
                    }
                )

            if OPTIONAL_STANDARD_SOURCE_PATH.exists():
                print("표준 CSV 출처 목록:")
                display(pd.read_csv(OPTIONAL_STANDARD_SOURCE_PATH))

        for spec in OPTIONAL_FACTOR_SPECS:
            variable = spec["변수"]
            if variable in standard_optional_vars:
                continue
            if variable == "경력단절여성인구" and "경력단절여성비율" in standard_optional_vars:
                continue
    ...
    md(
        ...
        과제 안내의 인구 증감률 항목은 보조 분석으로 확인합니다.
        `data/optional_kosis/population_change_panel_2015_2025.csv`가 있으면 자연증가율과 출산율의 관계를 간단히 시각화합니다.
        """
    ),
    code(
        """
        POPULATION_CHANGE_PATH = DATA_DIR / "optional_kosis" / "population_change_panel_2015_2025.csv"
        ...
        if POPULATION_CHANGE_PATH.exists():
            population_change_df = pd.read_csv(POPULATION_CHANGE_PATH)
            population_change_df.columns = [nfc(str(col).strip()) for col in population_change_df.columns]
            population_change_df["지역"] = population_change_df["지역"].map(normalize_region)
            population_change_df["연도"] = pd.to_numeric(population_change_df["연도"], errors="coerce").astype("Int64")
            population_change_df["자연증가율"] = pd.to_numeric(population_change_df["자연증가율"], errors="coerce")
            population_change_df = population_change_df.dropna(subset=["지역", "연도"]).copy()
            population_change_df["연도"] = population_change_df["연도"].astype(int)
            population_change_df = keep_sido_rows(population_change_df)

            growth_coverage = (
                population_change_df.dropna(subset=["자연증가율"])
                .groupby("연도")["지역"]
                .nunique()
                .rename("자연증가율_지역수")
                .reset_index()
            )
            display(growth_coverage.tail(12))

            latest_growth_year = int(population_change_df.dropna(subset=["자연증가율"])["연도"].max())
            latest_growth = population_change_df[population_change_df["연도"] == latest_growth_year].dropna(subset=["자연증가율"])
            latest_growth = latest_growth.sort_values("자연증가율", ascending=False)

            plt.figure(figsize=(12, 5))
            sns.barplot(data=latest_growth, x="지역", y="자연증가율", order=latest_growth["지역"])
            plt.axhline(0, color="black", linewidth=1)
            plt.title(f"{latest_growth_year}년 시도별 자연증가율")
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.show()

            birth_growth_df = pd.merge(
                birth_df[["지역", "연도", "출산율"]],
                population_change_df[["지역", "연도", "자연증가율"]],
                on=["지역", "연도"],
                how="inner",
            )
            latest_joint_year = int(birth_growth_df.dropna()["연도"].max())
            latest_joint = birth_growth_df[birth_growth_df["연도"] == latest_joint_year].dropna()

            plt.figure(figsize=(7, 5))
            sns.regplot(data=latest_joint, x="출산율", y="자연증가율", scatter_kws={"alpha": 0.8})
            for _, row in latest_joint.iterrows():
                plt.text(row["출산율"], row["자연증가율"], row["지역"], fontsize=8)
            plt.axhline(0, color="black", linewidth=1)
            plt.title(f"{latest_joint_year}년 출산율과 자연증가율")
            plt.tight_layout()
            plt.show()

            print("출산율-자연증가율 상관계수:", round(latest_joint["출산율"].corr(latest_joint["자연증가율"]), 3))
            print("자연증가율은 출생뿐 아니라 사망 구조도 함께 반영하므로, 합계출산율의 보조 지표로 해석합니다.")
